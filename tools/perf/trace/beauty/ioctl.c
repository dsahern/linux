...
static size_t ioctl__scnprintf_cmd(unsigned long cmd, const char *cmd_nr_str,
				   char *bf, size_t size, bool show_prefix)
{
	...
	if (cmd_nr_str && size > printed)
		return printed + scnprintf(bf + printed, size - printed,
					   ", %#x, %s, %#x)", type, cmd_nr_str, sz);
...
size_t syscall_arg__scnprintf_ioctl_cmd(char *bf, size_t size, struct syscall_arg *arg)
{
	...
	if (file != NULL) {
		...
		if (file->pathname)
			cmd_nr_str = syscall_arg__ioctl_cmd_lookup(file->pathname, _IOC_NR(cmd));
	}

	return ioctl__scnprintf_cmd(cmd, cmd_nr_str, bf, size, arg->show_string_prefix);
}
